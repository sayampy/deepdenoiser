...
export class DeepFilterNet {
    ...
    private initStates(): { states: ort.Tensor | ort.Tensor[], attenLimDb: ort.Tensor } {
        let states: ort.Tensor | ort.Tensor[];

        if (this.hasSingleStateTensor) {
            states = new ort.Tensor('float32', new Float32Array(this.statesLen), [this.statesLen]);
        } else {
            states = [];
            const stateInputsMeta = metadata.inputs.filter(i => this.stateInputNames.includes(i.name));
            for (const meta of stateInputsMeta) {
                const size = meta.shape.reduce((a, b) => a * b, 1);
                states.push(new ort.Tensor('float32', new Float32Array(size), meta.shape));
            }
        }
    ...
    public async denoise(
        ...
        const hopSizeDivisiblePaddingSize = (this.hopSize - (origLen % this.hopSize)) % this.hopSize;
        ...
        for (let start = 0; start < paddedAudio.length; start += this.hopSize) {
            ...
            const frame = paddedAudio.slice(start, end);
            ...
            const inputFrame = new ort.Tensor('float32', inputFrameData, [this.hopSize]);

            const feeds: ort.InferenceSession.Feeds = { 'input_frame': inputFrame };
        ...
        const concatenatedLength = enhancedFrames.reduce((sum, arr) => sum + arr.length, 0);
...
async function main() {
    ...
    console.log(`Denoising complete. Enhanced audio length: ${enhancedAudio.length}`);
